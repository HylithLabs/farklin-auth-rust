...
use crate::web::{routes_auth, routes_devices, routes_session};
...
async fn main() -> Result<()> {
	...
	// -- Define Routes
	// `/api/session` and the `/api/sessions*` active-devices routes are the
	// only ones requiring a valid session — signup/signin/refresh/logoff
	// must stay reachable while logged out.
	let routes_protected = routes_session::routes(mm.clone())
		.merge(routes_devices::routes())
